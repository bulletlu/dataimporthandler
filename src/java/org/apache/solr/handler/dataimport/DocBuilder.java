  ...
  private VariableResolver getVariableResolver() {
    try {
      ...
      for (Entity entity : dataImporter.getConfig().getEntities()) {
          ...
          Map<String, Object> entryIndexerNamespace = new HashMap<String, Object>();
          //if (lastIndex != null && lastIndex instanceof Date) {
          if (lastIndex != null) {
            //indexerNamespace.put(key, lastIndex);
        	  entryIndexerNamespace.put(SolrWriter.LAST_INDEX_KEY, lastIndex);
          } else  {
            //indexerNamespace.put(key, EPOCH);
        	  entryIndexerNamespace.put(SolrWriter.LAST_INDEX_KEY, EPOCH);
          }
          indexerNamespace.put(entity.getName(), entryIndexerNamespace);
        }
